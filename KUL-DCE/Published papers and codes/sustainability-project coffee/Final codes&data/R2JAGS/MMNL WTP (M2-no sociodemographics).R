...
############                  MMNL[M2.1-no sociodemographics] MODEL-WTP SPACE #############################
###########                   DIC calculation ##############
...
cat("model{
    ...
    mftbl[i,1] <- mft + bnep[1]*nepmatrix[i,1]+ basb[1]*asbmatrix[i,1]+ baho[1]*ahomatrix[i,1]
    
    mftbl[i,2] <- mbl + bnep[2]*nepmatrix[i,1] + basb[2]*asbmatrix[i,1] + baho[2]*ahomatrix[i,1]
    ...
    file="C:/KU_Leuven/DCE/project_coffee/scripts/WINBUGS/R2JAGS/MMNL2_nodemographs_WTP.jags")
...
mmnldcof =    #             
  list(choices=dset, J=J,N=N,S= S,faitradematrix=faitradematrix, biolabmatrix=biolabmatrix,nepmatrix=nepmatrix,
       ...
       pitaly=pitaly, tdessert=tdessert, tmokka =tmokka, cpricematrix=cpricematrix,  
       T=structure(.Data = c(1,0,0, 1),.Dim=c(2,2)), FB=structure(.Data = c(1,0,0, 1),.Dim=c(2,2)))
# initial values
...
mmnl2par<-c('asc0est','brazilorig','indonesiaorig','netherlandsprod','italyprod',
            'mbl','blab_nep', 'blab_asb', 'blab_aho', 'mft', 'ftrade_nep', 'ftrade_asb','ftrade_aho','sigma.ftrade','sigma.biolable', 
...
m2 <- R2jags::jags(data = mmnldcof, inits = inits, mmnl2par, n.chains = 3, n.iter = 100000, n.burnin = 50000, n.thin=1,
                   model.file = "C:/KU_Leuven/DCE/project_coffee/scripts/WINBUGS/R2JAGS/MMNL2_nodemographs_WTP.jags")
