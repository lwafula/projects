...
monitor<-c('asc0estwtp','tastedesertwtp','tastemokkawtp','brazilorigwtp','indonesiaorigwtp','netherlandsprodwtp',
           'italyprodwtp', 'price','biolabwtp','fairtradewtp')

# running the model
mnlwtp = run.jags(model = mnlwtpcoffeeset, modules = 'glm',data = dcof,
                  monitor = c(monitor,'D' ,'deviance', 'pd', 'popt', 'dic', 'ped'),  
burnin = 50000, n.chains = 3, sample = 100000, 
inits = list(inits1,inits2,inits3))
mnlwtp$summary$statistics
xtable::xtable(cbind(round(mnlwtp$summary$statistics[,1],3),round(mnlwtp$summary$statistics[,2],3),
                     ifelse(abs(mnlwtp$summary$statistics[,1]/mnlwtp$summary$statistics[,2])>1.96,'*','')))
beepr::beep(sound = 8) #ppc=0.24(0.43)

# calculate deviances at the mean parameters
meanpars=rbind(as.data.frame(mnlwtp$mcmc[[1]][,colnames(mnlwtp$mcmc[[1]][,monitor])]),as.data.frame(mnlwtp$mcmc[[2]][,colnames(mnlwtp$mcmc[[1]][,monitor])]),
               as.data.frame(mnlwtp$mcmc[[3]][,colnames(mnlwtp$mcmc[[1]][,monitor])]))
...
pD=as.numeric(mnlwtp$summary$statistics['D',1])-m1(meanpars0)$Dhat
DIC<- pD+as.numeric(mnlwtp$summary$statistics['D',1])

# option 2
d=rbind(as.data.frame(mnlwtp$mcmc[[1]][,'D']),as.data.frame(mnlwtp$mcmc[[2]][,'D']),as.data.frame(mnlwtp$mcmc[[3]][,'D']))
