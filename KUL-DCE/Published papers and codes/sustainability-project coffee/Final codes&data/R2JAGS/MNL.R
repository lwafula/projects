...
cat("model{
    ...
    file="C:/KU_Leuven/DCE/project_coffee/scripts/WINBUGS/R2JAGS/MNL.jags")
    ...
    mnlpars<-c('asc0est','tastedesert','tastemokka','brazilorig','indonesiaorig','netherlandsprod','italyprod', 'price','biolab','fairtrade')
    
    
    # running the model in JAGS
   mnl <- jags(data = dcof, inits = list(inits1, inits2, inits3), mnlpars,
                  n.chains = 3, n.iter = 100000, n.burnin = 50000, n.thin=1, 
           model.file = "C:/KU_Leuven/DCE/project_coffee/scripts/WINBUGS/R2JAGS/MNL.jags")

cbind(round(mnl$BUGSoutput$summary[,1],3),round(mnl$BUGSoutput$summary[,2],3), 
      ifelse(abs(mnl$BUGSoutput$summary[,1]/mnl$BUGSoutput$summary[,2])>1.96,'*',''))
