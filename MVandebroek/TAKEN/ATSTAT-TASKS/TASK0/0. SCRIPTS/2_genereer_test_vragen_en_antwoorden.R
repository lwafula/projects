...
# include functions from 0_source_taak0.R

source("0. SCRIPTS/0_source_taak0_LM.R")

### Laad vragenpool en student numbers 
vragenpool_NED_ENG <- read_excel(path ="1.FILES\\vragen_questions_TASK0.xlsx")

#Read in Q-numbers 
user_info <- read_excel(path = "1.FILES\\user_info with coding_TASK0.xlsx")
...
for(i in 1:I) {
  ...
  indfolder= paste0("W:TASK0\\2.QUESTIONS")
  ...
  if (LABEL == "TSTAT") {
    ...
    questions_IQ[i,2] = paste("Cursist ID: ", as.character(ID), "\n\n", 
                              paste0("Gebruik de data in https://feb.kuleuven.be/public/",personID,"/TASK0/1.DATA/data",idnewid,".txt"), "\n",
    ...
    # write.table(questions_IQ[i, -1], file = filepathBx, quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      questions_IQ[i,2] = paste("Student ID: ", as.character(ID), "\n\n", 
                                paste0("Use the data in https://feb.kuleuven.be/public/",personID,"/TASK0/1.DATA/data",idnewid,".txt"), "\n",
