...
# include functions from 0_source_tryout.R

source("0. SCRIPTS/0_source_tryout_LM.R")

### Laad vragenpool en student numbers 
vragenpool_NED_ENG <- read_excel(path ="1.FILES\\vragen_questions_TRYOUT.xlsx")

#Read in Q-numbers 
user_info <- read_excel(path = "1.FILES\\user_info with coding_TRYOUT.xlsx")
...
for(i in 1:I) {
  ...
  if (LABEL == "TSTAT") {
    ...
    questions_IQ[i,2] = paste("Cursist ID: ", as.character(ID), "\n\n", 
                              paste0("Use the data in ", "https://kuleuven-my.sharepoint.com/:t:/r/personal/leonard_maaya_kuleuven_be1/Documents/ATSTAT-TASKS/TRYOUT/2.INDIVIDUAL/1.DATA/data",idnewid,".txt")
                              , "\n",
...
write_xlsx(solutions_IQ, path = "1.FILES\\solutions_tryout.xlsx")
write_xlsx(questions_IQ, path = "1.FILES\\indquestions_tryout.xlsx")
