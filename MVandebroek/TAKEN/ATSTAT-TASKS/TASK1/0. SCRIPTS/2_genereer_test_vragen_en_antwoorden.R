...
user_info <- read_excel(path = "1.FILES/user_info with coding_TASK1.xlsx")
...
for(i in 1:I) {
  ...
  indfolder= paste0("W:\\TASK1\\2.QUESTIONS")
  ...
  if (LABEL == "TSTAT") {
    ...
    questions_IQ[i,2] = paste("Cursist ID: ", as.character(ID), "\n\n", 
                              paste0("Gebruik de data in https://feb.kuleuven.be/public/",personID,"/TASK1/1.DATA/data",idnewid,".txt"), "\n",
    ...
    # write.table(questions_IQ[i, -1], file = filepathBx, quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      questions_IQ[i,2] = paste("Student ID: ", as.character(ID), "\n\n", 
                                paste0("Use the data in https://feb.kuleuven.be/public/",personID,"/TASK1/1.DATA/data",idnewid,".txt"), "\n",
...
write_xlsx(solutions_IQ, path = "1.FILES\\solutions_taak1.xlsx")
write_xlsx(questions_IQ, path = "1.FILES\\indquestions_taak1.xlsx")
