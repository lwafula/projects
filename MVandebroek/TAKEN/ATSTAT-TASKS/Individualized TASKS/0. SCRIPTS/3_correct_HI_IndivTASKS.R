...
responses <- read_excel("1.FILES\\responses_test_IndivTASKS.xlsx") |>
...
solutions <- read_xlsx("1.FILES\\solutions_IndivTASKS.xlsx") |>
  rename("User.Name" = `ID`)

## read in user info
user_info <- read_excel("1.FILES\\user_info with coding_Indiv_TASKS.xlsx") |>
  rename("User.Name" = `Username`)
...
write_xlsx(all_scores, path = "1.FILES\\overallgrades_all_IndivTASKS.xlsx")
write.table(all_scores, file = "1.FILES\\overallgrades_all_IndivTASKS.txt", 
