...
# include functions from 0_source_taak2.R

source("0. SCRIPTS/0_source_taak2.R")

### Laad vragenpool en student numbers 
vragenpool_NED_ENG <- read_excel(path ="1.FILES\\vragen_questions_taak2.xlsx")
...
for(i in 1:I) {
  ...
  if (LABEL == "TSTAT") {
    ...
    questions_IQ[i,2] = paste("Cursist ID: ", as.character(ID), "\n\n", 
                              paste0("Gebruik de data in https://feb.kuleuven.be/public/",personID,"/TASK2/1.DATA/data",idnewid,".txt"), "\n",
    ...
    questions_IQ[i,2] = paste("Student ID: ", as.character(ID), "\n\n", 
                              paste0("Use the data in https://feb.kuleuven.be/public/",personID,"/TASK2/1.DATA/data",idnewid,".txt"), "\n",
...
write_xlsx(questions_IQ, path = "1.FILES\\indquestions_taak2.xlsx")
